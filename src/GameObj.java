...
public abstract class GameObj {
    ...
    //Move method for when the object needs to not hit wall.
    public void moveToWall(Wall wall) {
    	if(this.willHitWall(wall)) {
    		Direction collisionDirection = this.hitWall(wall);
    		if(collisionDirection == Direction.LEFT) {
    			this.px += ((wall.POS_X + wall.TILE_WIDTH) - (this.px));
    			this.py += this.vy;
    		}
    		else if(collisionDirection == Direction.RIGHT) {
    			this.px += (wall.POS_X - (this.width + this.px));
    			this.py += this.vy;
    		}
    		else if(collisionDirection == Direction.UP) {
    			this.px += this.vx;
    			this.py += ((wall.POS_Y + wall.TILE_HEIGHT) - (this.py));
    			
    		}
    		else if(collisionDirection == Direction.DOWN) {
    			this.px += this.vx;
    			this.py += (wall.POS_Y - (this.height + this.py));
    		}
    		else {
    			this.move();
    		}
    ...
    public boolean intersects(Wall that) {
    	return (this.px + this.width >= that.POS_X
                && this.py + this.height >= that.POS_Y
                && that.POS_X + that.TILE_WIDTH >= this.px 
                && that.POS_Y + that.TILE_HEIGHT >= this.py);
    }
    ...
    public boolean willHitWall(Wall wall) {
    	int thisNextX = this.px + this.vx;
        int thisNextY = this.py + this.vy;
    
        return (thisNextX + this.width >= wall.POS_X
            && thisNextY + this.height >= wall.POS_Y
            && wall.POS_X + wall.TILE_WIDTH >= thisNextX 
            && wall.POS_Y + wall.TILE_HEIGHT >= thisNextY);
    }
     ...
     *  
     * @param Optional, pass in a wall object.
     * @return Direction of impending wall, null if all clear.
     */
    public Direction hitWall(Wall wall) {
    	if(wall == null) {
    		...
    		if (this.px + this.width + this.vx > wall.POS_X && this.px + this.width <= wall.POS_X) {
                return Direction.RIGHT;
            }
    		else if (this.px + this.vx < wall.POS_X + wall.TILE_WIDTH && this.px >= wall.POS_X + 
    				wall.TILE_WIDTH) {
               return Direction.LEFT;
            }
    		
            if (this.py + this.height + this.vy > wall.POS_Y && this.py + this.height 
            		<= wall.POS_Y) {
                return Direction.DOWN;
            } else if (this.py + this.vy < wall.POS_Y + wall.TILE_HEIGHT && this.py >= wall.POS_Y + 
    				wall.TILE_HEIGHT) {
                return Direction.UP;
            } else {
                return null;
